...
class OrderFeedListPage(BasePage):
    ...
    def check_click_to_order_and_view_details(self, driver):
        self.open_page(driver, Constants.ORDER_FEED_LIST_URL)
    ...
    def get_orders_numbers_from_feed_list(self, driver):
        self.open_page(driver, Constants.ORDER_FEED_LIST_URL)
        result_list = self.get_text_from_element(OrderFeedListPageLocators.FIRST_ORDER_LOCATOR)
        return result_list

    @allure.step('Получить заказы "В работе"')
    def get_order_list_processing(self, driver):
        driver.execute_script("window.open('https://stellarburgers.nomoreparties.site/feed')")
    ...
    def check_total_order_counter(self, driver):
        self.open_page(driver, Constants.ORDER_FEED_LIST_URL)
    ...
    def check_today_order_counter(self, driver):
        self.open_page(driver, Constants.ORDER_FEED_LIST_URL)
